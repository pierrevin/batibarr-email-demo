...
export function EmailList({
  ...
  return (
    <section className="w-[220px] border-r border-zinc-200 bg-white flex flex-col shrink-0">
      <div className="border-b border-zinc-200 px-2.5 py-2 space-y-2">
        ...
        {items.length > 0 && !listLoading ? (
          <div className="space-y-1">
            ...
            <div className="text-[10px] text-zinc-500">{readCount} relu{readCount !== 1 ? "s" : ""}</div>
          </div>
        ) : null}
      ...
      {listError ? <div className="px-2.5 py-2 text-xs text-red-600">{listError}</div> : null}

      <div className="overflow-auto flex-1">
        {items.length === 0 && !listLoading ? (
          <div className="px-2.5 py-4 text-xs text-zinc-600">Aucun email à afficher.</div>
        ) : null}

        {visibleItems.length === 0 && items.length > 0 && !listLoading ? (
          <div className="px-2.5 py-4 text-xs text-zinc-600">Aucun email dans ce filtre.</div>
        ) : null}
